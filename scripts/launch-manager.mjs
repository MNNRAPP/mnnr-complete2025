...
async function collectChecklistGaps() {
  const results = [];
...
async function main() {
  ...
  if (checklistGaps.length === 0) {
    console.log('All tracked checklists are ✅ complete.');
  } else {
    checklistGaps.forEach(({ source, items }) => {
      console.log(`⚠️  ${source.label}`);
      items.slice(0, 5).forEach((item) => {
        const scope = item.heading ? ` (${item.heading})` : '';
        console.log(`   • ${item.text}${scope}`);
      });
      if (items.length > 5) {
        console.log(`   … and ${items.length - 5} more pending tasks`);
      }
      console.log('');
    });
    console.log('   ↳ Reference these documents to knock out remaining TODOs.');
  }
